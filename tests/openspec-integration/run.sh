...
done < <(find "$TEST_ROOT/cases" -maxdepth 1 -type f -name 'test_*.sh' -print | LC_ALL=C sort)

if (( selected == 0 )); then
    printf '[ERROR] 没有找到匹配的测试：%s\n' "${FILTER:-<all>}" >&2
    exit 2
fi
...
if [[ "$require_no_skips" == 1 ]] && (( skipped > 0 )); then
    printf '[ERROR] AUTOAI_REQUIRE_NO_SKIPS=1，但有 %d 个测试被跳过\n' "$skipped" >&2
    exit 1
fi
exit 0
